...
import Lib (application, logoutLink)
import Servant
import Test.Syd
import Test.Syd.Wai
...
spec = waiClientSpecWith application $ do
  describe "authenticating" $ do
    ...
    describe "valid requests" $ do
      ...
      describe "authorization via permissions" $ do
        describe "AliBaba (has both FooPermission and BarPermission)" $ do
          it "/fooo can be accessed" $ do
            resp <- request methodPost "/login" [("Content-Type", "application/x-www-form-urlencoded")] "username=AliBaba&password=OpenSesame"
        ...
        describe "FooUser (only has FooPermission)" $ do
          it "/fooo can be accessed" $ do
            resp <- request methodPost "/login" [("Content-Type", "application/x-www-form-urlencoded")] "username=FooUser&password=pass123"
            liftIO $ context "login response status" $ responseStatus resp `shouldBe` found302

            resp' <- get "/fooo"
            liftIO $ context "resource can be accessed" $ responseStatus resp' `shouldBe` ok200

          it "/baar can't be accessed" $ do
            resp <- request methodPost "/login" [("Content-Type", "application/x-www-form-urlencoded")] "username=FooUser&password=pass123"
            liftIO $ context "login response status" $ responseStatus resp `shouldBe` found302

            resp' <- get "/baar"
            liftIO $ context "resource can't be accessed" $ do
              responseStatus resp' `shouldBe` found302
              snd <$> find (\h -> fst h == hLocation) (responseHeaders resp') `shouldBe` Just "login?ref=denied"

        describe "BarUser (only has BarPermission)" $ do
          it "/baar can be accessed" $ do
            resp <- request methodPost "/login" [("Content-Type", "application/x-www-form-urlencoded")] "username=BarUser&password=pass123"
            liftIO $ context "login response status" $ responseStatus resp `shouldBe` found302

            resp' <- get "/baar"
            liftIO $ context "resource can be accessed" $ responseStatus resp' `shouldBe` ok200

          it "/fooo can't be accessed" $ do
            resp <- request methodPost "/login" [("Content-Type", "application/x-www-form-urlencoded")] "username=BarUser&password=pass123"
            liftIO $ context "login response status" $ responseStatus resp `shouldBe` found302
        ...
        describe "NooUser (does not have any permissions)" $ do
          it "/fooo can't be accessed" $ do
            resp <- request methodPost "/login" [("Content-Type", "application/x-www-form-urlencoded")] "username=NooUser&password=pass123"
            liftIO $ context "login response status" $ responseStatus resp `shouldBe` found302
